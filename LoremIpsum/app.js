...
form.addEventListener("submit",function(e){
  e.preventDefault();//By default it submits to server and then value is lost
  //const value = amount.value; // returns a string instead of number cause when we input it inputs as string like in java
  const value=parseInt(amount.value);
  const random=Math.floor(Math.random()*text.length);
  //if empty
  //-1
  //>9 cause we have only 9 para in text array
